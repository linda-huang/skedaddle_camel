...
let hit_wall (st : t) (pos : Position.t) (dir : int) (radius : int) = 
  ...
  (* Graphics.moveto (fst st.top_left_corner + 3) (snd st.top_left_corner + 30);
     ...
     Graphics.fill_rect 
     (fst st.top_left_corner + 3) (snd st.top_left_corner + 30) 33 20;  
     Graphics.set_color Graphics.red;
     Graphics.draw_string 
     (if (hit_corner st (Position.init_pos (fst two_corners)) || hit_corner st 
...
let move_camel_ice (st : t) (camel : Camel.t) : Camel.t = 
  ...
  | Some (ncol, nrow, ndir) -> begin
      (* Graphics.moveto (100) (60);
         ...
         Graphics.draw_string 
         (" ice goal " ^ (string_of_int ncol) ^ " " ^ string_of_int nrow); *)
      match head_tile, tail_tile with 
      | Valid (hc, hr), Valid (tc, tr) -> 
        (* Graphics.draw_string 
           (" curr head " ^ (string_of_int hc) ^ " " ^ string_of_int hr); 
           Graphics.draw_string 
           (" curr tail " ^ (string_of_int tc) ^ " " ^ string_of_int tr);  *)
        if hc = tc && hr = tr then 
...
let in_tile (st : t) (camel : Camel.t) (col, row) = 
  ...
  (* Graphics.draw_string ("abs " ^ (string_of_int (Int.abs (camel.pos.x - cx) + 
                                                 Int.abs (camel.pos.y - cy)))); *)
...
let handle_hit_portal (st : t) (camel : Camel.t) (col, row) = 
  ...
  (* Graphics.moveto (100) (60);
     ...
     Graphics.set_color Graphics.white;
     Graphics.draw_string 
     (" in tile " ^ (string_of_bool (in_tile st camel (col, row))));
     Graphics.draw_string 
     (" teleport " ^ (string_of_bool (camel.teleport)));  *)
...
let hit_power_tile (st : t) (pos : Position.t) =
  (* Graphics.moveto 
     (fst st.top_left_corner - 50) (snd st.top_left_corner + 30);
     Graphics.set_color Graphics.black; 
     Graphics.fill_rect 
     (fst st.top_left_corner - 50) (snd st.top_left_corner + 30) 250 20;   *)
  ...
  | Valid (cc, rc) -> begin
      ...
      | Power_Path Ice ->
        (* Graphics.moveto 
           ...
           Graphics.set_color Graphics.black; 
           Graphics.fill_rect 
           (fst st.top_left_corner + 50) (snd st.top_left_corner + 30) 33 20;  
           Graphics.set_color Graphics.white;
      ...
      | Power_Path Mud ->
        ...
        (* Graphics.moveto 
           (fst st.top_left_corner + 100) (snd st.top_left_corner + 30);
           Graphics.set_color Graphics.black; 
           Graphics.fill_rect 
           (fst st.top_left_corner + 100) (snd st.top_left_corner + 30) 33 20;  
           Graphics.set_color Graphics.white;
      ...
      | Power_Path Portal ->
        (* | Power_Path Portal, _ -> *)
        (* Graphics.moveto 
           (fst st.top_left_corner + 150) (snd st.top_left_corner + 30);
           Graphics.set_color Graphics.black; 
           Graphics.fill_rect 
           (fst st.top_left_corner + 150) (snd st.top_left_corner + 30) 33 20;  
           Graphics.set_color Graphics.white;
